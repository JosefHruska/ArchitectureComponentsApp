...
fun DatabaseQuery.observe(): Flowable<DataSnapshot> {
    return Flowable.create<DataSnapshot>({
        val query = this.build()
        val listener = query.addValueEventListener(object : ValueEventListener {
            override fun onCancelled(databaseError: DatabaseError) {
                it.onError(Throwable(databaseError.message)) // Data no longer exist or permission denied //Nevim no
            }
...
fun DatabaseQuery.exists(): Flowable<Boolean?> {
